...
import json, csv, os, re, threading, difflib, uuid, time, ipaddress, zipfile, sqlite3 as _sqlite3

# Resolve database path from environment or default to project-relative
_DB_PATH = os.environ.get("NOC_TOOLKIT_DB_PATH", os.path.join(os.path.dirname(__file__), "noc_toolkit.db"))
...
def inject_enabled_pages():
    """Inject enabled_pages and app_version into all templates."""
    return {
...
# Tables to include in backup, ordered so parent tables come before children.
# The sessions table is excluded (transient data).
BACKUP_TABLES = [
    "users", "audit_log", "kb_articles",
    "app_settings", "page_settings",
    "wlc_dashboard_settings", "wlc_summer_settings", "ap_inventory_settings",
    "solarwinds_settings", "cert_sync_settings",
    "wlc_dashboard_samples", "wlc_summer_samples", "ap_inventory", "solarwinds_nodes",
    "certificates", "ise_nodes",
    "jobs", "job_events",
    "bulk_ssh_jobs", "bulk_ssh_results", "bulk_ssh_templates", "bulk_ssh_schedules",
    "change_windows", "change_events",
]
...
@require_superadmin
def admin_backup():
    """Backup & Restore page (superadmin only)."""
    import sqlite3
    conn = sqlite3.connect(_DB_PATH)
    cursor = conn.cursor()
    ...
    conn.close()
    return render_template("admin_backup.html", table_counts=table_counts)


@app.route("/admin/backup/export", methods=["POST"])
@require_superadmin
def admin_backup_export():
    """Generate and download a ZIP backup of all database tables."""
    import sqlite3
    conn = sqlite3.connect(_DB_PATH)
    cursor = conn.cursor()
    ...
    log_audit(
        session["username"], "backup_export",
        resource=f"{len(row_counts)} tables, {sum(row_counts.values())} rows",
    ...
    resp.headers["Content-Disposition"] = f"attachment; filename=noc_toolkit_backup_{timestamp}.zip"
    return resp
...
def admin_backup_restore():
    """Restore database tables from an uploaded ZIP backup."""
    import sqlite3
    ...
    if total_size > MAX_RESTORE_SIZE:
        flash("Backup file exceeds the 100 MB size limit.", "error")
        return redirect(url_for("admin_backup"))

    if "manifest.json" not in zf.namelist():
        flash("Invalid backup: missing manifest.json.", "error")
        return redirect(url_for("admin_backup"))

    try:
        manifest = json.loads(zf.read("manifest.json"))
    except (json.JSONDecodeError, KeyError):
        flash("Invalid backup: corrupt manifest.json.", "error")
        return redirect(url_for("admin_backup"))

    for key in ("tables", "row_counts", "backup_format"):
        if key not in manifest:
            flash(f"Invalid backup: manifest missing '{key}'.", "error")
            return redirect(url_for("admin_backup"))

    for table in manifest["tables"]:
        if table not in BACKUP_TABLES:
            flash(f"Invalid backup: unknown table '{table}'.", "error")
            return redirect(url_for("admin_backup"))

    for table in manifest["tables"]:
        if f"{table}.csv" not in zf.namelist():
            flash(f"Invalid backup: missing {table}.csv.", "error")
            return redirect(url_for("admin_backup"))
    ...
    try:
        ...
        for table in BACKUP_TABLES:
            ...
            csv_text = zf.read(f"{table}.csv").decode("utf-8")
        ...
        log_audit(
            session["username"], "backup_restore_failed",
            resource=str(exc), user_id=session["user_id"],
        )
        return redirect(url_for("admin_backup"))
    finally:
    ...
    log_audit(
        session["username"], "backup_restore",
        resource=f"{len(restored_counts)} tables, {total_rows} rows",
        user_id=session["user_id"],
    )
    flash(f"Backup restored successfully: {len(restored_counts)} tables, {total_rows} rows.", "success")
    return redirect(url_for("admin_backup"))
...
def get_kb_articles_for_user(user_id: int):
    ...
    conn = kb_sqlite.connect(_DB_PATH)
